...
import EditOrderForm from "@/components/ui-components/data/orders/forms/EditOrderForm";
import ViewOrderComponent from "@/components/ui-components/data/orders/other/ViewOrderComponent";
...
import { calculateTotalCustomers } from "@/utils/data/orders/calculateTotalCustomers";
...
export default function OrdersPage({
  ...
  const stats = {
    unique_orders: response?.data?.length,
    customers_count: calculateTotalCustomers(response?.data || []),
    products_count: calculateTotalOrderedProducts(response?.data || []),
    total_value: calculateTotalOrdersValue(response?.data || []),
  };
  return (
    <div className="flex items-start  ">
      ...
      {isSidenavOpen && (
        <div
          ...
          className="hs-overlay hs-overlay-open:-translate-x-0 border-t    hidden translate-x-full fixed right-0 top-14  transition-all duration-300 transform h-full max-w-[74rem] w-full z-[80] bg-white border-s dark:bg-neutral-800 dark:border-neutral-700 [--body-scroll:true] [--overlay-backdrop:false]"
        >
          ...
          <div className="  scrollbar-thumb-gray-300 scrollbar-thin scrollbar-thumb-rounded-full active:scrollbar-thumb-sky-400 scrollbar-thumb-rounded-full hover:scrollbar-thumb-blue-500 scrollbar-track-gray-100 h-full overflow-auto">
            {addItem?.includes("create_order") ? (
              <div>
                <CreateOrderForm customers={customers} />
              </div>
            ) : addItem?.includes("edit_order") ? (
              <div>
                <EditOrderForm />
              </div>
            ) : (
              <div>
                {response?.data?.map((order) => (
                  <div key={order?._id}>
                    <ViewOrderComponent order={order} />
                  </div>
                ))}
              </div>
            )}
