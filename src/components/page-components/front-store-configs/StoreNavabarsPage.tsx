...
import NavbarConfiguration from "@/components/ui-components/front-store-configs/navbars/NavbarConfiguration";
import { toast } from "react-toastify";

import NavbarPreview from "@/components/ui-components/front-store-configs/navbars/NavbarPreview";
import NavbarTemplates from "@/components/ui-components/front-store-configs/navbars/NavbarTemplates";
...
export default function StoreNavabarsPage({ response }: { response: any }) {
  const {
    changePreviewDevice,
  ...
  useEffect(() => {
    setFrontStoreConfigNavbarInputs(response);
  }, [response]);

  const handleSubmit = async () => {
    // Handle form submission logic
    const pok = "front_store_configuration";
    const key_to_update = "navbar";
    const value_to_update = frontStoreConfigNavbarInputs;
    const id = toast.loading("Updating navbar information...");
    ...
    if (res.response === "ok") {
      toast.update(id, {
        render: "Navbar updated successfully",
        type: "success",
        isLoading: false,
      });
    } else {
    ...
    <div className="relative ">
      <div className="border-b flex justify-between items-center px-6 border-gray-200 dark:border-neutral-700">
        ...
        <div className="items-center   flex   space-x-4">
          <button
            type="button"
            onClick={() => setFrontStoreConfigNavbarInputs({})}
          ...
          <div className="inline-flex rounded-lg shadow-sm">
            ...
            {frontStoreConfigNavbarInputs?.template_name && (
              ...
              >
                {frontStoreConfigNavbarInputs?.template_name?.replaceAll(
                  "_",
                  " "
                )}
              </button>
