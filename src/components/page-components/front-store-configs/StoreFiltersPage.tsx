...
import FilterCreationPanel from "@/components/ui-components/front-store-configs/filters/FilterCreationPanel";
import FiltersPreview from "@/components/ui-components/front-store-configs/filters/FiltersPreview";
import FiltersTemplates from "@/components/ui-components/front-store-configs/filters/FiltersTemplates";
import {
  useFrontStoreConfig,
...
export default function StoreFiltersPage({ response }: { response: any }) {
  const {
    changePreviewDevice,
  ...
  useEffect(() => {
    setFrontStoreSidePanelFilterConfigInputs(response);
  }, [response]);

  const handleSubmit = async () => {
    // Handle form submission logic
    const pok = "front_store_configuration";
    const key_to_update = "filters";
    const value_to_update = frontStoreSidePanelFilterConfigInputs;
    const id = toast.loading("Updating filters information...");
    ...
    if (res.response === "ok") {
      toast.update(id, {
        render: "Filters updated successfully",
        type: "success",
        isLoading: false,
      });
    } else {
      toast.update(id, {
        render: "Failed to update filters information",
        type: "error",
        isLoading: false,
      });
    }
    ...
    <div className="relative ">
      <div className="border-b flex justify-between items-center px-6 border-gray-200 dark:border-neutral-700">
        ...
        <div className="items-center   flex   space-x-4">
          <button
            type="button"
            onClick={() => resetFrontStoreSidePanelFilterConfigInputs()}
          ...
          <div className="inline-flex rounded-lg shadow-sm">
            ...
            {frontStoreSidePanelFilterConfigInputs?.template_name && (
              <button
                type="button"
                className="py-2 px-3 inline-flex capitalize justify-center items-center gap-2 -ms-px first:rounded-s-lg first:ms-0 last:rounded-e-lg text-sm font-medium focus:z-10 border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-800"
              >
                {frontStoreSidePanelFilterConfigInputs?.template_name?.replaceAll(
                  "_",
                  " "
                )}
              </button>
          ...
          <div className="inline-flex border gap-3 border-gray-200 rounded-lg p-1 dark:border-neutral-700">
            <button
              ...
              className={`inline-flex flex-shrink-0 justify-center items-center p-1 rounded text-gray-500 hover:bg-blue-100 bg-${
                previewDevice === "mobile" ? "blue-600" : "transparent"
              } text-${
                previewDevice === "mobile" ? "white" : ""
              }   focus:z-10 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-neutral-500 dark:hover:bg-blue-900 dark:hover:text-blue-200`}
            >
            ...
            <button
              ...
              className={`inline-flex flex-shrink-0 justify-center items-center p-1 rounded text-gray-500 hover:bg-blue-100 bg-${
                previewDevice === "tablet" ? "blue-600" : "transparent"
              } text-${
                previewDevice === "tablet" ? "white" : ""
              }   focus:z-10 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-neutral-500 dark:hover:bg-blue-900 dark:hover:text-blue-200`}
            >
            ...
            <button
              ...
              className={`inline-flex flex-shrink-0 justify-center items-center p-1 rounded text-gray-500 hover:bg-blue-100 bg-${
                previewDevice === "desktop" ? "blue-600" : "transparent"
              } text-${
                previewDevice === "desktop" ? "white" : ""
              }   focus:z-10 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-neutral-500 dark:hover:bg-blue-900 dark:hover:text-blue-200`}
            >
