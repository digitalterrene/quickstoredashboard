...
export default function CreateAutoMessage() {
  ...
  const handleSubmit = async () => {
    const response = await addToSet(user._id, user?._id, user?.token, {
      key_to_update: "auto_messages",
      value_to_update: {
        data_type: "auto_messages",
        message_type: activeOption,
        message: inputs,
        publisher: {
          name: user?.username,
          email: user?.email,
    ...
    if (response?.response !== "ok") {
      ...
      toast.success(`${response?.response === "ok" ? "success" : "error"}`);
    }
  };
  return (
