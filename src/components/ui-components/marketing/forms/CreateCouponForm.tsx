...
const CreateCouponForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const id = toast.loading("Creating a new coupon...");
    const res: any = await addToSet(user?._id, user?._id, user?.token, {
      key_to_update: "coupons",
      value_to_update: {
        data_type: "coupons",
        ...formData,
      },
    ...
    if (res.response === "ok") {
      toast.update(id, {
        render: "New coupon created successfully",
        type: "success",
        isLoading: false,
      });
    } else {
      toast.update(id, {
        render: "Failed to create a new coupon",
        type: "error",
        isLoading: false,
      });
    }
        ...
        <div className="mt-4 ">
          <div className="flex items-center gap-5">
            ...
            <div className="flex gap-x-2">
              <label htmlFor="uploadFile-CreateCouponForm">
                <input
                  type="file"
                  onChange={async (e) => {
                  ...
                  id="uploadFile-CreateCouponForm"
                  className="hidden"
                />
